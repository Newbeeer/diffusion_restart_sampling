...
def edm_sampler(
    ...
    init_steps=18, sigma_min=0.002, sigma_max=80, rho=7,
    S_churn=0, S_min=0, S_max=float('inf'), S_noise=1,
    pfgmpp=False, ensemble=1, restart_steps="[18]", restart="[1]", restart_S = "[80]", restart_end="[0.0]", **kwargs
):
    try:
        restart_steps = json.loads(restart_steps)
        restart = json.loads(restart)
        restart_S = json.loads(restart_S)
        restart_end = json.loads(restart_end)
    except:
        print("restart_steps", restart_steps)
        pass
    # print(restart_steps)
    ...
    def one_step(t_cur, t_next, gamma, x_cur, second_order=False):
        # print(second_order)
        ...
        # Euler step.
        # preconditions
        #print(x_hat, t_hat, net(x_hat, t_hat, class_labels))
    ...
    oldt = torch.Tensor([sigma_max]).to(latents.device)
    if pfgmpp:
        x_next = latents.to(torch.float64)
    else:
        x_next = latents.to(torch.float64) * oldt
    eps = torch.Tensor([1e-3]).to(latents.device)
    for (times, steps, start, end) in zip(restart, restart_steps, restart_S, restart_end):
        print("restart", times, steps, start, end)
        for restart_iter in range(times):
            new_steps = get_steps(end, start, steps, rho)
            if new_steps[0] > oldt:
                x_next = x_next + randn_like(x_next) * ((new_steps[0] ** 2 - oldt ** 2).sqrt()) * S_noise
            else:
                assert (new_steps[0] + 1e-2 > oldt)
            for i, (t_cur, t_next) in enumerate(zip(new_steps[:-1], new_steps[1:])):
                if t_cur < eps:
                    continue
                x_next = one_step(t_cur, t_next, 0, x_next, second_order=(True if t_next > eps else False))
                oldt = t_next
            
    if oldt < eps:
        x_next = x_next + randn_like(x_next) * ((eps ** 2 - oldt ** 2).sqrt()) * S_noise
        oldt = eps
    if oldt > eps:
        x_next = one_step(oldt, eps, 0, x_next, second_order=True)
        oldt = eps
    x_next = one_step(oldt, 0, 0, x_next, second_order = False)
    return x_next
...
@click.option('--steps', 'init_steps',      help='Initial generate steps', metavar='INT',                          type=click.IntRange(min=1), default=18, show_default=True)
@click.option('--restart_steps',      help='Number of sampling steps per restart', metavar='STR', type = str, default='0', show_default=True)
...
@click.option('--restart_S', 'restart_S',             help='restart starting point', metavar='STR', type = str, default='0', show_default=True)
@click.option('--restart_end', 'restart_end',             help='restart ending point', metavar='STR', type = str, default='0', show_default=True)
...
@click.option('--restart',             help='restart number', metavar='STR',                           type=str, default='0', show_default=True)

def main(ckpt, end_ckpt, outdir, subdirs, seeds, class_idx, max_batch_size, save_images, pfgmpp, aug_dim, ensemble, use_pickle, device=torch.device('cuda'), **sampler_kwargs):
    """Generate random images using the techniques described in the paper
    "Elucidating the Design Space of Diffusion-Based Generative Models".
    ...
    for ckpt_dir in stats:
        ...
        if use_pickle:
            name = sampler_kwargs['name']
            temp_dir = os.path.join(outdir, f'ckpt_000000_{name}')
