...
def edm_sampler(
        ...
        pfgmpp=False, ensemble=1, inter_steps=18, restart=0, restart_value=-1,
):
    def get_steps(min_t, max_t, num_steps, rho):

        if min_t != 0:
            step_indices = torch.arange(num_steps, dtype=torch.float, device=latents.device)
            t_steps = (max_t ** (1 / rho) + step_indices / (num_steps - 1) * (
                    min_t ** (1 / rho) - max_t ** (1 / rho))) ** rho
        else:
            num_steps -= 1
            step_indices = torch.arange(num_steps, dtype=torch.float, device=latents.device)
            t_steps = (max_t ** (1 / rho) + step_indices / (num_steps - 1) * (
                    sigma_min ** (1 / rho) - max_t ** (1 / rho))) ** rho
    ...
    # step = 18:
    restart_list = {14: [restart, 0.29644, 3]}
        ...
        if i + 1 in restart_list.keys():
            ...
            for restart_iter in range(restart_list[restart_idx][0]):
                ...
                if pfgmpp:
                    ...
                    # print(moves.shape, (z + moves.view(len(x_next), 1) * dir[:, N:]).norm(p=2, dim=1))
                    x_next = x_next + moves * dir[:, :N].view_as(x_next)
...
@click.option('--inter_steps', help='Number of intermediate sampling steps', metavar='INT', type=click.IntRange(min=0),
...
@click.option('--S_max', 'S_max', help='Stoch. max noise level', metavar='FLOAT', type=click.FloatRange(min=0),
              default=0, show_default=True)
@click.option('--S_noise', 'S_noise', help='Stoch. noise inflation', metavar='FLOAT', type=float, default=0,
              show_default=True)
...
@click.option('--aug_dim', help='additional dimension', metavar='INT', type=click.IntRange(min=2), default=128,
              show_default=True)
@click.option('--ensemble', help='ensemble number', metavar='INT', type=click.IntRange(min=1), default=1,
              show_default=True)
@click.option('--restart', help='restart number', metavar='INT', type=click.IntRange(min=0), default=0,
              show_default=True)
@click.option('--restart_value', help='restart value', metavar='FLOAT', type=click.FloatRange(min=-1), default=-1,
              show_default=True)
def main(ckpt, end_ckpt, outdir, subdirs, seeds, class_idx, max_batch_size, save_images, pfgmpp, aug_dim, ensemble,
         use_pickle, name, device=torch.device('cuda'), **sampler_kwargs):
    """Generate random images using the techniques described in the paper
    "Elucidating the Design Space of Diffusion-Based Generative Models".
    ...
    for ckpt_dir in stats:
        ...
        if wandb_enabled:
            wandb.init(project="sde_restart", entity="goodeat", settings=wandb.Settings(start_method='fork'),
