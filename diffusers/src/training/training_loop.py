        ...
        for round_idx in range(num_accumulation_rounds):
            with misc.ddp_sync(ddp, (round_idx == num_accumulation_rounds - 1)):
                ...
                if stf:
                    ...
                    vec = batch_images.view(len(batch_images), -1)
                    diff = vec - vec.unsqueeze(1)
                    diff_norm = diff.norm(p=2, dim=2)
                    #print(diff.norm(p=2, dim=2))
                    print("mean: ", diff_norm.mean())
                    v, idx = torch.sort(diff_norm, dim=1, descending=False)
                    min_dis = diff_norm[torch.arange(len(diff_norm)), idx[:, 1]]
                    print("min: ", min_dis)
                    print("mean of min: ", min_dis.mean())
                    exit(0)
