...
class StableDiffusionParticlePipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
    r"""
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps[:-1]):
                ...
                if do_classifier_free_guidance:
                    ...
                    h_t = (distance.mean(dim=1, keepdim=True) * num_images / (num_images - 1)) ** 2 / np.log(num_images)
                    ...
                    grad_phi = 2 * weights * diff / h_t * sigma * 2 * coeff
                    ...
                    # get the norm of grad_phi
                    norm_grad_phi = torch.norm(grad_phi.view(len(grad_phi), -1), p=2, dim=-1, keepdim=True)
                    # get the norm of noise_pred_text
                    norm_noise_pred_text = torch.norm(noise_pred_text.view(len(noise_pred_text), -1), p=2, dim=-1, keepdim=True)
                    print("sigma:", sigma.item())
                    print("norm_grad_phi", norm_grad_phi)
                    print("norm_noise_pred_text", norm_noise_pred_text)

                    # noise_pred_text_sum = noise_pred_text.sum(dim=0, keepdim=True)
                    # noise_pred_text_sum_diff = noise_pred_text_sum - noise_pred_text
                    # noise_pred_text_diff_mean = noise_pred_text_sum_diff / (num_images - 1)
                    #
                    # # get the norm of noise_pred_text_diff_mean
                    # norm_noise_pred_text_diff_mean = torch.norm(noise_pred_text_diff_mean.view(len(noise_pred_text_diff_mean), -1), p=2, dim=-1, keepdim=True)
                    # print("norm_noise_pred_text_diff_mean", norm_noise_pred_text_diff_mean)
                    ...
                    for restart_iter in range(restart_list[restart_idx][1]):
                        ...
                        for j, new_t in enumerate(new_t_steps[:-1]):
                            ...
                            if do_classifier_free_guidance:
                                ...
                                distance = torch.norm(diff, p=2, dim=-1, keepdim=True)
                                num_images = latents_vec.shape[0]
                                h_t = (distance.mean(dim=1, keepdim=True) * num_images / (num_images - 1)) ** 2 / np.log(
                                    num_images)

                                grad_phi = 2 * torch.exp(- (distance**2 / h_t)) * diff / h_t * sigma * 2 * coeff
                                grad_phi = grad_phi.sum(dim=1)
                                grad_phi = grad_phi.view_as(latents)
                                ...
                                noise_pred = noise_pred_text + guidance_scale * (
                                            noise_pred_text - noise_pred_uncond) - grad_phi
