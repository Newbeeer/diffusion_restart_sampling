...
class SDEScheduler(SchedulerMixin, ConfigMixin):
    """
    ...
    def step(
    ...
    ) -> Union[SDESchedulerOutput, Tuple]:
        """
        ...
        sigma_t = torch.sqrt((1-alpha_prod_t) / alpha_prod_t)
        sigma_t_prev = torch.sqrt((1-alpha_prod_t_prev) / alpha_prod_t_prev)
        x_bar_t = sample / torch.sqrt(alpha_prod_t)

        noise = torch.randn_like(x_bar_t) * torch.sqrt(2*sigma_t) * torch.sqrt(sigma_t - sigma_t_prev)
        x_bar_t_prev = x_bar_t + 2 * (sigma_t_prev - sigma_t) * pred_epsilon + noise

        prev_sample = x_bar_t_prev * torch.sqrt(alpha_prod_t_prev)
        ...
        return SDESchedulerOutput(prev_sample=prev_sample, pred_original_sample=pred_original_sample)
    ...
    def add_noise_between_t(
        ...
        print(f"add noise {timesteps_1} -> {timesteps_2}")
